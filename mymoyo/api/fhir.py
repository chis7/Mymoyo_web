...
def _base_resource(instance):
    return {
        ...
        'meta': {
            'profile': ['https://mymoyo.local/fhir/StructureDefinition/mymoyo-resource'],
        },
    }
...
def _location_for_facility(facility):
    ...
    if facility.code:
        resource['identifier'] = [{
            'system': 'https://mymoyo.local/fhir/NamingSystem/facility-code',
            'value': facility.code,
        }]
...
def _healthcare_service(service):
    ...
    resource.update({
        ...
        'name': service.name,
        'identifier': [{
            'system': 'https://mymoyo.local/fhir/NamingSystem/service-code',
            'value': service.code,
        }],
    })
...
def _person_resource(profile):
    ...
    resource.update({
        ...
        'identifier': [{
            'system': 'https://mymoyo.local/fhir/NamingSystem/client-reference',
    ...
    if profile.person_identity_id:
        resource.setdefault('extension', []).append({
            'url': 'https://mymoyo.local/fhir/StructureDefinition/person-identity',
    ...
    if resource['resourceType'] == 'Practitioner':
        ...
        if profile.facility_id:
            resource.setdefault('extension', []).append({
                'url': 'https://mymoyo.local/fhir/StructureDefinition/worker-facility',
...
def _provenance_resource(audit_log):
    ...
    resource.update({
        ...
        'target': [{
            'identifier': {
                'system': f'https://mymoyo.local/source/{audit_log.app_label}/{audit_log.model_name}',
